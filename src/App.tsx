...
export default function MetronomeApp() {
  ...
  const [rhythmTrainer, setRhythmTrainer] = useState<RhythmTrainerConfig>(() => 
    getStorageItem(STORAGE_KEY_RHYTHM_TRAINER, { enabled: false, playBars: 3, muteBars: 1 }, JSON.parse)
  );
  ...
  const { isPlaying, setIsPlaying, visualBeat, ensureAudioContext, measureCount, isMeasureMuted } = useMetronome(
    bpm, beatsPerMeasure, subdivision, soundPreset, stepStates,
    { rhythmTrainer, onMeasureComplete: handleMeasureComplete }
  );
